    ...
    try {
        ...
        try {
            ...
            if (replicateResponse instanceof ReadableStream) {
                const errorText = await streamToString(replicateResponse);
                console.error("Replicate API returned a ReadableStream with content:", errorText);
                return res.status(500).json({ error: "Image generation service returned an error", details: errorText });
            }
